...
char* trace(int num); // generate NUM backtraces. flip sign to reverse order. do not free(), temporary values
char* tracef(FILE *fp, int num); // generate NUM backtraces and write them. flip sign to reverse order. do not free(), temporary values
...
#elif KIT_GCC && !KIT_WASM && !KIT_MINGW // maybe KIT_LINUX is enough?
#include <execinfo.h>  // backtrace, backtrace_symbols
#elif KIT_WINDOWS // && !defined __TINYC__
...
char **backtrace_symbols(void *const *list,int size) {
    ...
    static THREAD char **symbols = 0; //[32][64] = {0};
    if( !symbols ) {
...
#if KIT_TCC && KIT_LINUX
    static char empty[1];
    return empty[0] = '\0', empty;
#endif
#if KIT_WASM // there is a stack overflow failure somewhere in the impl below
        ...
        *strrchr( address, ')') = '\0'; *(address - 1) = '\0';
        ...
        if( dladdr(stacks[i], &info) && info.dli_sname ) {
            const char *dmgbuf = info.dli_sname[0] != '_' ? NULL :
                 ifdef(KIT_CPP, __cxa_demangle(info.dli_sname, NULL, 0, NULL), info.dli_sname);
            strcpy( demangled, dmgbuf ? dmgbuf : info.dli_sname );
            symbols[i] = demangled;
            ifdef(KIT_CPP, dmgbuf && free( (void*)dmgbuf ) );
        }
#endif
        if( symbols[i] )
...
char* tracef( FILE *fp, int traces ) {

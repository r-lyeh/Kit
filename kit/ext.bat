@echo off
if not exist "kit" exit /b && rem cd /d "%~dp0"
...
if "%1"=="gen" (
    ...
    rem generate kit.h file
    echo // auto-generated file. do not edit  > ext.h
    echo // list of extensions included here >> ext.h
    echo. >> ext.h
    for /f "delims=" %%i in ('dir /s /b /ad * ^| sort ') do @if exist "%%i\kit.h" (
    set DIR=%%i
    set DIR=!DIR:%CD%\=!
    set DIR=!DIR:\=/!
    echo #if __has_include^("!DIR!/kit.h"^) >> ext.h
    echo #         include "!DIR!/kit.h" >> ext.h
    echo #endif >> ext.h
    )

    rem generate kit.hh file
    echo // auto-generated file. do not edit  > ext.hh
    echo // list of extensions included here >> ext.hh
    echo. >> ext.hh
    rem echo #ifdef __cplusplus >> ext.hh
    for /f "delims=" %%i in ('dir /s /b /ad * ^| sort ') do @if exist "%%i\kit.hh" (
    set DIR=%%i
    set DIR=!DIR:%CD%\=!
    set DIR=!DIR:\=/!
    echo #if __has_include^("!DIR!/kit.hh"^) >> ext.hh
    echo #         include "!DIR!/kit.hh" >> ext.hh
    echo #endif >> ext.hh
    )
...
:dir_ext
rem curl -s "https://api.github.com/search/repositories?q=topic:kit1+&sort=updated&order=desc" > exts.0
    curl -s "https://api.github.com/search/repositories?q=topic:kit1+&sort=name&order=asc" > exts.0

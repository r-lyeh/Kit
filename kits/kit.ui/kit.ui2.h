...
API int     ui2_collapse_end(void);
API int   ui2_panel_end(void);
API int   ui2_tray(void); // can be nested within a window (local tray), or freestanding (desktop tray)
...
API int     ui2_menu_end(void);
API int   ui2_tray_end(void);
API int ui2_window_end(void);
...
API int ui2_enabled(void);

API int ui2_hide(bool);
API int ui2_hidden(void);

API int ui2_active(void);
API int ui2_hovered(void);
...
void ui2_tick(void) {
    ...
    // reset hue
    ui2_hue = 0;
    ImColor color = ImColor_HSV(ui2_hue / 7.0f, 0.7f, 0.7f, 1);
    ui2_hue32 = igGetColorU32_Vec4(color.Value);
...
typedef map_(const char*,int*) windows_t;
static windows_t windows = {less, hash};
int ui2_window_shown(const char *name) {
    int* *found = map_find(windows, stable(name));
    return found ? !!(**found & UI2_OPEN) : 0;
}
int ui2_window_show(const char *name, int vis) {
    int* *found = map_find(windows, stable(name));
    if( found ) { if( vis ) **found |= UI2_OPEN; else **found &= ~UI2_OPEN; }
    return !!vis;
}
int ui2_window(const char *label, int *flags) {
    assert( flags );
    int* *found = map_find_or_add(windows, stable(label), NULL);
    *found = flags;
...
int ui2_adjust_nextpanel(const char *label, int flags) {
    ...
    static map_(const char *, int) set = {less, hash};
    int* counter = map_find_or_add(set, stable(label), 0); int first = *counter == 0;
    map_add(set, stable(label), *counter+1);
    ...
    if( first ) {
        ...
        int spacing = 16;
        static int items = 0;
        const ImVec2 PAD = has_menubar ? ImVec2(2,5) : ImVec2(0,0);
...
int ui2_notify(const char *label, const char *body) {
    ...
    return (igNotify)(type, strchr("ki",type) ? 3.0 : -1.0, label, body), 1;
}
...
/// - column reordering
/// - hidden columns
float igTableFullRowBegin(void) {
...
static int UI2_LABEL_1OF2(const char *label) {
    ...
    if(!has_label) label = "";
        ...
        if( tooltip ) snprintf(copy, 512, "%.*s %s", (int)(tooltip - label), label, UI2_ICON(INFO/*HELP*/) );
        ...
        UI2_LABEL_MIN = igGetItemRectMin(); UI2_LABEL_MIN.y -= BAR_HEIGHT;
...
static int UI2_LABEL_2OF2( int rc ) {
    ...
    if( 1 ) {
        UI2_LABEL_MAX = igGetItemRectMax(); UI2_LABEL_MAX.y += BAR_HEIGHT;
...
int ui2_string(const char *label, char **str) {
    ...
    if(!*str) *str = REALLOC(0,1); //strcatf(str, "", "");
...
int ui2_slider2(const char *label, float *pct, const char *caption) {
    ...
    igGetIO_Nil()->ConfigWindowsMoveFromTitleBarOnly = true;
//    igPushStyleColor_Vec4(ImGuiCol_PlotHistogram, *igGetStyleColorVec4(ImGuiCol_PlotLinesHovered));
    ...
    if( igIsItemHovered(0) && mouse('l') ) {
        ImVec2 click = igGetMousePos(), src = igGetItemRectMin();
...
//    igPopStyleColor(1);
    //igGetIO_Nil()->ConfigWindowsMoveFromTitleBarOnly = false;
...
int ui2_color3(const char *label, unsigned *color32) { //[0..255]
    ImVec4 color = igColorConvertU32ToFloat4((ImU32)*color32);
...
int ui2_buttons(int buttons, /*labels*/...) {
    ...
    if( buttons >= COUNTOF(labels) ) os.die("ui2_buttons() capacity exceeded");
        va_list list;
...
int ui2_toolbar(const char *label, int icons, ...) {
    ...
    const char *labels[16] = {0};
    if( icons >= COUNTOF(labels) ) os.die("ui2_toolbar() capacity exceeded");
        va_list list;
        va_start(list, icons);
...
int ui2_radio(const char *label, int *selected, int icons, ...) {
    ...
    const char *labels[16] = {0};
    if( icons >= COUNTOF(labels) ) os.die("ui2_radio() capacity exceeded");
        va_list list;
        va_start(list, icons);
        for( int i = 0; i < icons; ++i ) {
            labels[i] = va_arg(list, const char*);
...
int ui2_bitmask(const char *label, int numbits, void *ptr) {
    ...
    for( int i = 0; i < numbits ; ++i ) {
        int j = ifdef(KIT_BIG,numbits - 1 - i,i);

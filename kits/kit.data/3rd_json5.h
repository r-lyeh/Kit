...
#ifdef  array_push
#define json5_array_count array_count
#define json5_array_push array_push
#define json5_array_free array_free
#else
typedef struct json5_hdr { int cap, num; json5 vec[]; } json5_hdr;
int json5_array_count(json5 *arr) {
    json5_hdr *hdr = (json5_hdr *)((char *)arr - offsetof(json5_hdr, vec));
    return arr ? hdr->num : 0;
}
json5* json5_array_push(json5 *arr, json5 val) {
    json5_hdr *hdr = (json5_hdr *)((char *)arr - offsetof(json5_hdr, vec));

    if (!arr) {
        int init_cap = 4;
        hdr = realloc(NULL, offsetof(json5_hdr, vec) + init_cap * sizeof(json5));
        if (!hdr) return arr;
        hdr->cap = init_cap;
        hdr->num = 0;
    }
    ...
    if (hdr->num == hdr->cap) {
        int new_cap = (int)(hdr->cap * 1.75);
        if (new_cap <= hdr->cap) new_cap = hdr->cap + 1; /* overflow guard */
        hdr = realloc(hdr, offsetof(json5_hdr, vec) + new_cap * sizeof(json5));
        if (!hdr) return arr; /* realloc failed, return old pointer unchanged */
        hdr->cap = new_cap;
    }
...
void json5_array_free(json5 *arr) {
    json5_hdr *hdr = (json5_hdr *)((char *)arr - offsetof(json5_hdr, vec));
    if(arr) hdr = realloc(hdr, 0);
}
#define json5_array_push(arr,val) ((arr) = json5_array_push((arr),(val)))
#define json5_array_free(arr) (json5_array_free(arr), (arr) = 0)
#endif
...
char *json5__parse_object(json5 *obj, char *p, char **err_code) {
    ...
    if( 1 /* *p == '{' */ ) { /* <-- for SJSON */
        ...
        while (*p) {
            ...
            if( node.type != JSON5_UNDEF ) {
                json5_array_push(obj->nodes, node);
                ++obj->count;
            }
...
char *json5__parse_value(json5 *obj, char *p, char **err_code) {
    ...
    if( is_string ) {
        ...
        while (*p) {
            ...
            if( elem.type != JSON5_UNDEF ) {
                json5_array_push(obj->array, elem);
                ++obj->count;
            }
            if (*p == ']') { ++p; break; }
        }
        ...
        for( int i = 0; labels[i]; ++i ) {
            if( !strncmp(p, labels[i], lenghts[i] ) ) {
                ...
                /**/ if( i >= 5 ) obj->type = JSON5_REAL, obj->floating = i >= 11 ? -INFINITY : i >= 9 ? INFINITY : i >= 7 ?  NAN :-NAN;
#else
                /**/ if( i >= 5 ) obj->type = JSON5_REAL, obj->floating = i >= 11 ? -INFINITY : i >= 9 ? INFINITY : i >= 7 ? -NAN : NAN;
#endif
        ...
        long long int lli;
        /**/ if( is_dbl ) sscanf( buffer, "%lf", &obj->floating );
...
void json5_free(json5 *root) {
    if( root->type == JSON5_ARRAY && root->array ) {
        for( int i = 0, cnt = json5_array_count(root->array); i < cnt; ++i ) {
    ...
    if( root->type == JSON5_OBJECT && root->nodes ) {
        for( int i = 0, cnt = json5_array_count(root->nodes); i < cnt; ++i ) {
...
void json5_write(FILE *fp, const json5 *o) {
    ...
    /**/ if( o->type == JSON5_NULL ) fprintf(fp, "%s", "null");
        ...
        /**/ if( isnan(o->floating) ) fprintf(fp, "%s", signbit(o->floating) ? "-nan" : "nan" );
        else if( isinf(o->floating) ) fprintf(fp, "%s", signbit(o->floating) ? "-inf" : "inf" );
        else fprintf(fp, "%1.8e", o->floating); // %1.8e from google:"randomascii 100 digits" ; %.4llf for compactness
    }
    #if 0
